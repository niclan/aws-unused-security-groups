...
# Get regions list once and for all
REGIONS=$(aws ec2 describe-regions | jq --raw-output '.Regions[].RegionName')
...
for region in $REGIONS; do
  ...
  for sg in $(aws rds describe-db-security-groups --region $region | jq --raw-output '.DBSecurityGroups[].EC2SecurityGroups[].EC2SecurityGroupId'); do
    printf "%s\t%s\n" "$region" "$sg">>sg-used-rds.txt
    printf "-"
  done
done
wc -l sg-used-rds.txt

printf "\nstep 6b: list security groups associated with rds instance vpcs "
for region in $REGIONS; do
  printf "=" >&2
  for sg in $(aws rds describe-db-instances --region $region | jq --raw-output '.DBInstances[].VpcSecurityGroups[].VpcSecurityGroupId'); do
    printf "%s\t%s\n" "$region" "$sg"
    printf "-" >&2
  done
done | sort -u > sg-used-rds-vpc.txt
wc -l sg-used-rds-vpc.txt
...
sort -u sg-used-*  >sg-used-sorted.txt
sort -u sg-all.txt >sg-all-sorted.txt
